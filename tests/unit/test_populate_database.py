...
from TMB.adapters.orm import metadata
...
def test_database_populate_select_all_comments(database_engine):
    ...
    with database_engine.connect() as connection:
        ...
        assert all_comments == [(1, 2, 1, 'Oh yes, this film has arrived New Zealand'),
                                (2, 1, 1, 'Yeah Freddie, good news'),
                                (3, 3, 1, "I hope it's as good as The Terminator!")]
...
def test_database_populate_select_all_movies(database_engine):
    ...
    with database_engine.connect() as connection:
        ...
        assert all_movies[nr_movies//2] == (51, 'Star Wars: Episode VII - The Force Awakens')
        assert all_movies[nr_movies-1] == (100, 'The Departed')
