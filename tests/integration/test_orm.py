...
def insert_movie(empty_session):
    empty_session.execute(
        'INSERT INTO movies (id,title,genre,description,director,actors,movie_date,runtime,rating,votes,revenue,metascore) VALUES '
        '(101, "test2", "GAL,Sci_Fi,News", "just for test", "demon from hell", "A,B,C", :date, 0,"",0,"","")',
        {'date': movie_date}
    )
...
def insert_genres(empty_session):
    empty_session.execute(
        'INSERT INTO genres (name) VALUES ("News"), ("GAL")'
    )
...
def test_saving_of_movie(empty_session):
    ...
    rows = list(empty_session.execute('SELECT movie_date, title, director, actors, id FROM movies'))
    date = movie_date
    assert rows == [(date, 'test2', None, '', 102)]
